...
class FinetuneConfig(TrainingConfigV2):
    def __init__(self):
        super().__init__()
        self.learning_rate = 2e-5  # Much lower LR for finetuning
        self.min_lr = 1e-6
...
def train():
    ...
    if args.data_file and os.path.exists(args.data_file):
        ...
        class InstructionDataset(Dataset):
            """Dataset for instruction tuning. Does NOT pack sequences. Uses padding."""
            def __init__(self, path, tokenizer, max_length):
                self.samples = []
                self.max_length = max_length
                
                print(f"Reading formatted instructions from {path}...")
                ...
                print(f"Loaded {len(self.samples)} valid sequences.")
        ...
        def collate_fn(batch):
            ...
            batch_max = min(max(len(x) for x in batch), train_config.max_seq_length)
            ...
            for tokens in batch:
                pad_len = batch_max - len(tokens)
                padded_tokens = tokens + [pad_id] * pad_len
                mask = [1] * len(tokens) + [0] * pad_len
                
                # Loss Masking: Set padding tokens to -100 so they don't affect loss
                label = tokens + [-100] * pad_len
        ...
        print("Building fine-tuning dataloaders (no packing)...")
        train_dataset = InstructionDataset(args.data_file, tokenizer, train_config.max_seq_length)
        ...
        if args.val_file and os.path.exists(args.val_file):
            val_dataset = InstructionDataset(args.val_file, tokenizer, train_config.max_seq_length)
    ...
    # ---- Model ----
    # ---------------------------------------------------------------------
    # Model & Checkpoint Loading (Fine-Tuning Logic)
    # ---------------------------------------------------------------------
    print("\n🧠 Initializing model for Fine-Tuning...")
    model = OptimusForCausalLM(model_config).to(device)

    # For compilation, wrap the model before loading states if using compile
    if train_config.use_compile and getattr(torch, "compile", None):
        print(f"⚙️  Compiling model (mode: {train_config.compile_mode})...")
        model = torch.compile(model, mode=train_config.compile_mode)

    # 1. Try to resume from an existing FINETUNE checkpoint
    latest_ckpt_path = os.path.join(train_config.checkpoint_dir, "latest_checkpoint.pt")
    base_model_ckpt = os.path.join("chkpoints_v2", "best_model.pt")
    
    ckpt = None
    if os.path.exists(latest_ckpt_path):
        print(f"📂 Resuming Fine-Tuning from {latest_ckpt_path}...")
        ckpt = torch.load(latest_ckpt_path, map_location="cpu", weights_only=False)
        model.load_state_dict(ckpt["model_state_dict"])
    elif os.path.exists(base_model_ckpt):
        print(f"📂 Starting Fine-Tuning from Pre-Trained Base Model: {base_model_ckpt}...")
        ckpt = torch.load(base_model_ckpt, map_location="cpu", weights_only=False)
        model.load_state_dict(ckpt["model_state_dict"])
        # We MUST set ckpt = None because we don't want to load the pre-training optimizer state!
        # We need a fresh optimizer for fine-tuning.
        ckpt = None
    else:
        print(f"❌ Error: Could not find base pre-trained model at {base_model_ckpt}")
        print("Please ensure your pre-training run generated a best_model.pt")
        sys.exit(1)
    ...
    if os.path.exists(latest_ckpt_path):
        print(f"\n📂 Resuming Fine-Tuning from checkpoint: {latest_ckpt_path}")
        ...
        print(f"\n📂 Starting Fine-Tuning from Pre-Trained Base Model: {base_model_ckpt}...")
        ckpt = torch.load(base_model_ckpt, map_location="cpu", weights_only=False)
        model.load_state_dict(ckpt["model_state_dict"])
        # Do NOT load optimizer or scheduler, they must start fresh for fine-tuning!
        print(f"   ✅ Base model weights loaded. Optimizer initialized fresh.")
    else:
        print(f"\n❌ Error: Could not find base pre-trained model at {base_model_ckpt}")
        print("Please ensure your pre-training run generated a best_model.pt before fine-tuning.")
        sys.exit(1)
